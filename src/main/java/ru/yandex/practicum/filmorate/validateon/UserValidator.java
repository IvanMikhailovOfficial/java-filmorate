...
public class UserValidator {
    public static void validate(User user) {
        ...
        if (user.getEmail() == null || user.getEmail().isBlank() || !user.getEmail().contains("@")) {
            ...
            //throw new ResponseStatusException(HttpStatus.valueOf(400), "Некорректный email");
        }

        if (user.getLogin().isBlank() || user.getLogin().contains(" ")) {
            log.debug("Неверный логин " + user);
            throw new ValidationException("Неверный логин");
            //throw new ResponseStatusException(HttpStatus.valueOf(400), "Неверный логин");
        }
        ...
        if (user.getBirthday().isAfter(LocalDate.now())) {
            ...
            //throw new ResponseStatusException(HttpStatus.valueOf(400), "Дата рождения не будет в будущем");
        }
    }
}
